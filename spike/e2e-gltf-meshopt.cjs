...
/**
 * EXT_meshopt_compression e2e — decode attributes + triangle indices.
 */

const fs = require('fs');
const path = require('path');
const os = require('os');
const { importGltfAsync, prepareMeshopt } = require('./importers/gltf.cjs');

const SRC = path.join(__dirname, 'fixtures', 'gltf-meshopt', 'meshopt.gltf');

(async () => {
  await prepareMeshopt();
  const library = path.join(os.tmpdir(), 'e2e-gltf-meshopt');
  ...
  console.log('[e2e-gltf-meshopt] SUCCESS', {
    mesh: meshUuid,
    verts: 3,
    indices: 3,
  });
})().catch((err) => {
  console.error('[e2e-gltf-meshopt] FAILED', err);
  process.exit(1);
});
